        ...
        pipe.once("finish", () => {
            ...
            fs.readFile(path.join(_templates, "default.html"), "utf8", (error, defaultTemplate) => {
                ...
                fs.readFile(path.join(_templates, "block.html"), "utf8", (error, blockTemplateRaw) => {
                    ...
                    fs.readFile(path.join(_templates, "innards.html"), "utf8", (error, blockInnardsTemplateRaw) => {
                        if(error) {
                            return callback(error);
                        }

                        let blockInnardsTemplate = mustache.render(defaultTemplate, { content: blockInnardsTemplateRaw });
                        ...
                        fs.mkdir(path.join(_site, "blocks"), (error) => {
                            ...
                            async.each(Object.keys(cryptoflow.blocks), (blockId, callback) => {
                                ...
                                let innardsHtml = mustache.render(blockInnardsTemplate, block);
                                ...
                                fs.mkdir(blockPath, (error) => {
                                    ...
                                    fs.writeFile(path.join(blockPath, "index.html"), html, (error) => {
                                        if(error) {
                                            return callback(error);
                                        }
                                        let blockInnardsPath = path.join(blockPath, "innards");
                                        fs.mkdir(blockInnardsPath, (error) => {
                                            if(error && error.code !== "EEXIST") {
                                                return callback(error);
                                            }
                                            fs.writeFile(path.join(blockInnardsPath, "index.html"), innardsHtml, callback);
                                        });
                                    });
                                });
                            }, (error) => {
                                ...
                                fs.readFile(path.join(_projectRoot, "README.md"), "utf8", (error, readmeMarkdown) => {
                                    ...
                                    fs.readFile(path.join(_templates, "index.html"), "utf8", (error, indexTemplateRaw) => {
                                        ...
                                        fs.writeFile(path.join(_site, "index.html"), indexHtml, (error) => {
                                            ...
                                            let readLicenseStream = fs.createReadStream(path.join(_projectRoot, "LICENSE.md"));
                                            let writeLicenseStream = fs.createWriteStream(path.join(_site, "LICENSE.md"));
                                            readLicenseStream.pipe(writeLicenseStream);
