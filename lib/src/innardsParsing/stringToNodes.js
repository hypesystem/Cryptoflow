...
function parseInnardsToNodes(block) {
    ...
    let initialVariables = block.inputs.map((input) => input.name);
    ...
    if(typeof block.innards == "function") {
        ...
        setUpFunctionInputs(variables, functionNode, initialVariables);

        return outputNode;
    }

    let statements = parseInnardsToAbstractSyntax(initialVariables, block.innards);
