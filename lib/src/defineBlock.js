...
module.exports = (config) => {
    ...
    block.render = (renderspace, inspectField, editField, inspectBlock) => {
        ...
        onBlockChange(() => {
            ...
            renderBlock(block, mainBlockX, mainBlockY, inspectBlock, renderspace);
            renderInputs(block, mainBlockX, mainBlockY, inputHeight, renderspace);
            renderOutput(block, renderspace, onOutputChange);

            doBlockCalculation(block).then(result => setOutput(result));
        });
...
function renderBlock(block, mainBlockX, mainBlockY, inspectBlock, renderspace) {
    // - square
...
function renderInputs(block, mainBlockX, mainBlockY, inputHeight, renderspace) {
    // render inputs
    ...
    if(firstInputEditableTextNode) {
        ...
        inputEditable.on("click", (d) => {
            ...
            let bytes = encode("hex", "buf", d.value);
...
function renderOutput(block, renderspace, onOutputChange) {
    // render output
