...
module.exports = (container) => {
    ...
    setTimeout(() => { //timeout until block is defined in the script block below. bad pattern.
        ...
        block.onOutputChange(() => inspectBlockModalOutput.innerText = encode("buf", inspectBlockEncoding, block.output.value));
    }, 1);
    let inspectBlock = (block) => {
        ...
        inspectBlockModal.querySelector(".inputs").innerHTML = block.inputs.map((input, i) => {
            ...
            <label for="input-${i}">
                Input (${input.name})
                <input type="text" name="input-${i}" class="input-box" value="${encode("buf", inspectBlockEncoding, input.value)}">
            </label>
            `;
        }).join("");
        ...
        block.inputs.forEach((input, i) => {
            ...
            inputElement.addEventListener("change", (e) => {
                block.setValue(input.name, encode(inspectBlockEncoding, "buf", inputElement.value));
            });
        });
        ...
        encodingSelector.addEventListener("change", (e) => {
            ...
            block.inputs.forEach((input, i) => {
                let inputElement = inspectBlockModal.querySelector(`[name=input-${i}]`);
                inputElement.value = encode("buf", inspectBlockEncoding, input.value);
            });
            inspectBlockModalOutput.innerText = encode("buf", inspectBlockEncoding, block.output.value);
        });
