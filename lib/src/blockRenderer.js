...
module.exports = (block, onBlockChange, onOutputChange, notifyChanged) => (renderspace, modalspace) => {
    ...
    onBlockChange(() => {
        ...
        renderedBlock.on("click", () => inspectBlock({
            id: block.id,
            name: block.name,
            inputs: block.inputs.map((input) => {
                return {
                    ...
                    setValue: (val) => block.setValue(input.name, val),
                    editable: true
                };
            }),
            getOutputValue: () => block.output.value,
            onOutputChange: (handler) => block.onOutputChange(handler),
            func: block.func
        }));
        ...
        onOutputChange(() => output.setValue(encode("buf", "hex", block.output.value)));
...
function renderInputs(block, mainBlockX, mainBlockY, inputHeight, renderspace, editField) {
    ...
    if(firstInputEditableTextNode) {
        ...
        inputEditable.on("click", (d) => editField({
            name: d.name,
            value: d.value,
            setValue: (val) => block.setValue(d.name, val)
        }));
...
function renderOutput(block, mainBlockX, mainBlockY, blockDimensions, renderspace) {
    ...
    if(block.output.value) {
        outputValueBlock.setValue(encode("buf", "hex", block.output.value));
    }
    else {
        outputValueBlock.setValue("loading...");
    }
